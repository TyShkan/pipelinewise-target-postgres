...
| hard_truncate                         | Boolean |            | (Default: False) When `hard_truncate` option is true then TRUNCATE SQL commands will be performed in Postgres to truncate tables. Otherwise, it would mark all rows with `_SDC_DELETED_AT IS NOT NULL` as deleted. It's achieved by continuously checking the  `_SDC_DELETED_AT` metadata column sent by the singer tap. Due to deleting rows requires metadata columns, `hard_truncate` option set to false automatically enables the `add_metadata_columns` option as well. |
